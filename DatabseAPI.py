...
async def get_all_ids():
  ...
  try:
    ...
    result = db._execute_query(
      "SELECT id FROM scrapped_data WHERE barcode_exists = 0 and is_uploaded = 0 and id > 3800",
      ['id']
    )
...
async def get_all_raw_ids():
  ...
  try:
    ...
    result = db._execute_query(
      "SELECT id FROM scrapped_data WHERE status = 'raw' and is_uploaded = 0 and id > 3800",
      ['id']
    )
    if not result:
